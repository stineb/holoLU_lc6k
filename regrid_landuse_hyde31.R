...
regrid.landuse.hyde <- function( year, scenario, grid.out, verbose=FALSE, aligned=FALSE ){ 
  ...
  dir <- "/alphadata01/bstocker/data/landuse_data/hyde3_1/"
  ...
  fil.orig <- paste(
                    dir,scenario,"/landuse_",sprintf("%06i",year),".nc",
                    sep=""
                    )
  nc <- open.nc(fil.orig)  
...
## -----------------------------------------------------------------
scenarios=c("hyde31_concave")
for (iscen in scenarios){
  yrs <- read.table( paste(
                           "/alphadata01/bstocker/data/landuse_data/hyde3_1/",
                           iscen,
                           "/yrlist.txt",
                           sep=""
                           ),
