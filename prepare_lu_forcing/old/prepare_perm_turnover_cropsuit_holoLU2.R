...
## Read GICEW file
nc <- open.ncdf( "/alphadata01/bstocker/data/card/forcings/lpx/soil/gicew/gicew_halfdeg.cdf" )
gicew <- get.var.ncdf(nc,"GICEW")
close.ncdf(nc)
        ...
        if ( is.na( index_in_olof[itim] ) ){
          ...
          if ( time_hyde[itim]<1700 ){
            ...
            if (!is.na(crop_hyde[ilon,ilat,itim])&&!is.na(scul_hurt[ilon,ilat])){
              ...
              if ( scul_hurt[ilon,ilat]!=1 || !is.na(civi_olof[ilon,ilat,4]) && civi_olof[ilon,ilat,4]==2) {
    ...
    if ( !is.na(crop_hyde[ilon,ilat,1])){
      ...
      for (itim in 1:len){
        ...
        if (perm_field[ilon,ilat,itim]==2){
          ...
          crop_suit[ilon,ilat,itim] <- min( gicew[ilon,ilat], fallow_factor[itim] * crop_hyde[ilon,ilat,itim] )
